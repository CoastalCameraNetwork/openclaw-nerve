...
import { AgentTimeline } from '../orchestrator/AgentTimeline';
import { useTaskStatus, useTaskSessions } from '../orchestrator/useOrchestrator';
...
export function TaskDetailDrawer({ task, onClose, onUpdate, onDelete, onExecute, onApprove, onReject, onAbort }: TaskDetailDrawerProps) {
  ...
  // Real-time agent status for orchestrated tasks
  const { status: agentStatus, loading: agentStatusLoading } = useTaskStatus(
    task?.labels.includes('orchestrated') && task.run ? task.id : null,
    3000 // Poll every 3 seconds
  );

  // Session history for review (shows captured output from completed sessions)
  const { sessions, loading: sessionsLoading } = useTaskSessions(
    task?.status === 'review' ? task.id : null,
        ...
        {task && (
          <>
            ...
            <div className="flex-1 overflow-y-auto px-3.5 py-3 space-y-3.5">
              ...
              {task.run && (
                <div className="border-t border-border/50 pt-3">
                  <h4 className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-2">
                    <Cpu size={12} />
                    Agent Execution
                  </h4>
                  ...
                  {/* Agent Timeline - Real-time status */}
                  <div className="bg-muted/30 rounded-lg p-3">
                    <AgentTimeline taskStatus={agentStatus} loading={agentStatusLoading} />
                  </div>
                </div>
              )}
              
              {/* Agent Timeline for orchestrated tasks (even without run) */}
              {task.labels.includes('orchestrated') && !task.run && (
                <div className="border-t border-border/50 pt-3">
                  <h4 className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-2">
                    <Cpu size={12} />
                    Agent Status
                  ...
                  <div className="bg-muted/30 rounded-lg p-3">
                    <AgentTimeline taskStatus={null} loading={false} />
                  </div>
                </div>
              )}

              {/* Session History - For REVIEW tasks, show captured session output */}
              {task.status === 'review' && (
                <div className="border-t border-border/50 pt-3">
                  <h4 className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-2">
                    <Archive size={12} />
                    Session History (Work Completed)
                  </h4>
                  ...
                  {sessionsLoading ? (
                    ...
                    <div className="text-xs text-muted-foreground py-4 bg-muted/30 rounded-md px-3">
                      No session history available. The agent may not have captured output yet.
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {sessions.map((session, idx) => (
                        <div key={idx} className="bg-muted/30 rounded-md p-3">
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <code className="text-[10px] text-muted-foreground">{session.label}</code>
                            ...
                            {session.capturedAt && (
                              <span className="text-[10px] text-muted-foreground">
                                {new Date(session.capturedAt).toLocaleString()}
                              </span>
                            )}
                          </div>
                          
                          {session.output && (
                            <div className="text-xs text-foreground whitespace-pre-wrap bg-background/50 rounded p-2 max-h-[300px] overflow-y-auto">
                              {session.output}
                            </div>
                          )}
                      ...
                      <div className="text-[10px] text-muted-foreground italic">
                        💡 Review the work above. If satisfied, click "Complete Task" to move to Done and close the session.
                      </div>
                    </div>
                  )}
                </div>
