  ...
  return (
    <div className="scan-lines relative h-screen flex flex-col overflow-hidden" data-booted={booted}>
      ...
      <div className="flex-1 flex gap-3 overflow-hidden min-h-0 px-2 pt-1.5 pb-2 sm:px-4 sm:pt-2 sm:pb-2">
        ...
        {!isCompactLayout && (
          <div className={(viewMode === 'kanban' || viewMode === 'orchestrator') ? 'hidden' : fileBrowserCollapsed ? 'contents' : 'h-full min-h-0'}>
        ...
        {isCompactLayout ? (
          <div className={`shell-panel flex-1 min-w-0 min-h-0 overflow-hidden rounded-[28px] boot-panel${(viewMode === 'kanban' || viewMode === 'orchestrator') ? ' hidden' : ''}`}>
            {chatContent}
          </div>
        ) : (
          <div style={{ display: (viewMode === 'kanban' || viewMode === 'orchestrator') ? 'none' : 'contents' }}>
            <ResizablePanels
